...
import depspace.confidentiality.DepSpaceConfidentialityImpl;
...
public class DepSpaceManager {
	...
	protected void setEventHandler(DepSpaceEventHandler eventHandler) {
        ...
        //TODO:este synchronized precisa????
	...
	private Object invokeTupleSpaceOperation(DepSpace tupleSpace, DepSpaceOperation operation, Object arg, Context ctx) throws DepSpaceException {
		...
		case INP:
			result = tupleSpace.inp((DepTuple) arg, ctx);
                        //System.out.println("share que vai retornar "+((DepTuple)result).getShare());
			break;
		case RD:
	...
	private DepSpaceServerLayer createTupleSpace(Properties properties) {
		...
		boolean useConfidentiality = DepSpaceProperties.getUseConfidentiality(properties);
                //System.out.println("USE CONF: "+useConfidentiality);
		if(useConfidentiality){
                    tupleSpace = new DepSpaceImplLayer(eventHandler, new DepSpaceConfidentialityImpl(DepSpaceConfiguration.realTimeNew));
                }else{
		...
		if(useConfidentiality) {
			try {
				...
				//System.out.println(processID +" Vai criar as coisas de conf!"+DepSpaceConfiguration.secretKeys.length);
                                PVSSEngine engine = PVSSEngine.getInstance(publicInfo);
				ConfidentialityScheme scheme = new ConfidentialityScheme(engine.getPublicInfo(),DepSpaceConfiguration.publicKeys,processID, DepSpaceConfiguration.secretKeys[processID]); 
