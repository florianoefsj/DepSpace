...
public class DepSpaceReplica extends DefaultRecoverable implements DepSpaceEventHandler {
    ...
    private synchronized void execute(byte[] command, MessageContext msgCtx, boolean priority) {
        DepSpaceRequest request = null;
        try {
            // Invoke operation
            request = new DepSpaceRequest(command, msgCtx);
            //System.out.println("Vai exec: " + request.operation +" from "+ request.context.invokerID);
