...
import depspace.confidentiality.ClientConfidentialityLayer;
...
public class DepSpaceAdmin {
    ...
    public synchronized DepSpaceAccessor createAccessor(Properties properties, boolean createSpace) throws DepSpaceException {
        ...
        if (tsName == null) {
            throw new DepSpaceException("The DepSpace name must be specified");
        }
        ...
        if (useConfidentiality) {
            ...
            try {
                ...
                scheme = new ConfidentialityScheme(engine.getPublicInfo(), DepSpaceConfiguration.publicKeys);
            } catch (Exception e) {
                e.printStackTrace();
            }
        ...
        // Create tuple space
        if (createSpace) {
            depSpaceClient.createSpace(properties);
        }
        return accessor;
    }
     ...
     * @throws DepSpaceException if specified Tuple Space wasn't created by this
     * admin
     */
    public synchronized void deleteSpace(String name) throws DepSpaceException {
        DepSpaceAccessor accessor = accessors.remove(name);
        if (accessor == null) {
            throw new DepSpaceException("The specified DepSpace wasn't created by this admin");
        }
        accessor.getTupleSpace().deleteSpace(name);
    }

    /**
     * Finalize the accessor and delete the logical tuple space related with
     * this accessor
     */
